 ...
 * Type definitions
 */
interface DeniedNotificationPayload {
  requestId: string;
}
...
interface OTRequest {
  ...
  respective_supervisor_denied_at?: string;
  respective_supervisor_denial_remarks?: string;
  status: string;
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const payload: DeniedNotificationPayload = await req.json();
    ...
    const result = await sendDenialNotifications(
      supabase,
      payload.requestId
    );
...
async function sendDenialNotifications(
  ...
  const { data: otRequest, error: otError } = await supabase
    .from('ot_requests')
    .select('id, ot_date, total_hours, reason, employee_id, supervisor_id, respective_supervisor_id, respective_supervisor_denied_at, respective_supervisor_denial_remarks, status')
    .eq('id', requestId)
    .single();
...
async function sendNotificationToDirectSupervisor(
  ...
  const denialRemarksPreview = otRequest.respective_supervisor_denial_remarks && otRequest.respective_supervisor_denial_remarks.length > 60
    ? `${otRequest.respective_supervisor_denial_remarks.substring(0, 60)}...`
    : otRequest.respective_supervisor_denial_remarks || 'No remarks provided';
