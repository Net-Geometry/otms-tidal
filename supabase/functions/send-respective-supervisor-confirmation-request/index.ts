...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const result = await sendConfirmationRequestNotifications(
      supabase,
      payload.requestId
    );
 ...
 */
async function sendConfirmationRequestNotifications(
  supabase: any,
  requestId: string
  ...
  const { data: otRequest, error: otError } = await supabase
    .from('ot_requests')
    .select('id, ot_date, total_hours, reason, employee_id, supervisor_id, respective_supervisor_id, supervisor_remarks, status')
    .eq('id', requestId)
    .single();
  ...
  const { data: respectiveSupervisor, error: respectiveSupervisorError } = await supabase
    .from('profiles')
    .select('id, full_name, employee_id')
  ...
  try {
    await sendNotificationToRespectiveSupervisor(
      supabase,
 ...
 */
async function sendNotificationToRespectiveSupervisor(
  _supabase: any,
  respectiveSupervisor: Supervisor,
  employee: Employee,
