...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const result = await sendConfirmationSuccessNotification(
      supabase,
      payload.requestId
    );
 ...
 */
async function sendConfirmationSuccessNotification(
  supabase: any,
  requestId: string
  ...
  const { data: otRequest, error: otError } = await supabase
    .from('ot_requests')
    .select('id, employee_id, ot_date, total_hours, status, supervisor_id, supervisor_confirmation_remarks')
    .eq('id', requestId)
    .single();
  ...
  const notificationPayload = {
    ...
    notification_type: 'ot_supervisor_confirmed', // New notification type
    data: {
      targetUrl,
