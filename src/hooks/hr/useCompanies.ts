...
function groupCompanies(companies: Company[]): CompaniesGrouped {
  const subsidiaries = companies.filter((company) => company.parent_company_id !== null);
  const parentIds = new Set(subsidiaries.map((company) => company.parent_company_id));

  const parent = companies.find(
    (company) => company.parent_company_id === null && parentIds.has(company.id)
  ) ?? null;
...
export function useCompaniesGrouped() {
  return useQuery({
    queryKey: [...COMPANIES_QUERY_KEY, 'grouped'],
    queryFn: async () => groupCompanies(await fetchCompanies()),
  });
}
