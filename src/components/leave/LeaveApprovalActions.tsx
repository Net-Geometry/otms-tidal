...
export function LeaveApprovalActions({
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <Button variant="destructive" onClick={() => setRejectOpen(true)} disabled={!!isApproving || !!isRejecting}>
        {rejectLabel}
      </Button>
      ...
      <Dialog open={approveOpen} onOpenChange={setApproveOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{approveLabel} Leave Request</DialogTitle>
            <DialogDescription>Optional remarks will be saved in the approval record.</DialogDescription>
          </DialogHeader>
          <div className="space-y-2">
            <Label>Remarks (Optional)</Label>
            <Textarea value={approveRemarks} onChange={(e) => setApproveRemarks(e.target.value)} placeholder="Add remarks..." />
          </div>
          <DialogFooter>
      ...
      <Dialog open={rejectOpen} onOpenChange={setRejectOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{rejectLabel} Leave Request</DialogTitle>
